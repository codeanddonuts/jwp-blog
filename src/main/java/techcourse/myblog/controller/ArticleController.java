...
public class ArticleController {
    ...
    public String read(@PathVariable long articleId, Model model) {
        return articleService.getArticle(articleId).map(article -> {
    ...
    public RedirectView update(
        ...
        articleService.update(articleId, new Article(getCurrentUser(session), title, coverUrl, contents));
        return new RedirectView("/articles/" + articleId);
    }

    @DeleteMapping("/articles/{articleId}")
    public RedirectView delete(@PathVariable long articleId, HttpSession session) {
        articleService.delete(articleId, getCurrentUser(session));
